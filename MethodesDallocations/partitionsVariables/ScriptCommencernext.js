...
y=y+25
    ...
    { var y=148
     ...
     .transition().delay(5000).remove()
     canvas.append("text").text("pouvant contenir ce processus suivant ")
     .attr("fill",'white').attr("font-size",14).attr("x",445).attr("y",260)
     .transition().delay(5000).remove()
     canvas.append("text").text("la dérniére insertion de la liste des ")
     .attr("fill",'white').attr("font-size",14).attr("x",455).attr("y",290)
     .transition().delay(5000).remove()
     canvas.append("text").text(" espaces libres ")
     .attr("fill",'white').attr("font-size",14).attr("x",510).attr("y",320)
     .transition().delay(5000).remove()
...
d3.select('#ajoutok').on('click',function(){
  ...
  btn.on('click',function(){
      window.location.href="PartitionVariablenextSansExemple.html"
  })
  bodyC.append("table").attr("id","example").attr("width","80%").style("position","relative").style("left","10%").attr("class","table table-bordered").attr("cellspacing","0");
  $('#example').DataTable( {
      ordering: false,
