...
y=y+25
    ...
    { var y=148
     ...
     .transition().delay(5000).remove()
     canvas.append("text").text("la partition choisi est la partition numero "+m.listeLibre.liste[i].identificateur)
     .attr("fill",'white').attr("font-size",14).attr("x",425).attr("y",200)
     .transition().delay(5000).remove()
     canvas.append("text").text("car elle représente le premier espace libre ")
     .attr("fill",'white').attr("font-size",14).attr("x",425).attr("y",230)
     .transition().delay(5000).remove()
     canvas.append("text").text("pouvant contenir ce processus a partir du")
     .attr("fill",'white').attr("font-size",14).attr("x",425).attr("y",260)
     .transition().delay(5000).remove()
     canvas.append("text").text("début de la liste des espaces libres ")
     .attr("fill",'white').attr("font-size",14).attr("x",445).attr("y",290)
     .transition().delay(5000).remove()
...
d3.select('#ajoutok').on('click',function(){
    ...
    function termineExemple()
    {
      ...
      if(result.value)
      {
        ...
        btn.on('click',function(){
            window.location.href="PartitionVariablefirstSansExemple.html"
        })
        bodyC.append("table").attr("id","example").attr("width","80%").style("position","relative").style("left","10%").attr("class","table table-bordered").attr("cellspacing","0");
        $('#example').DataTable( {
            ordering: false,
   ...
   d3.select('#exemple').on('click',function(){
    d3.select("#terminer").on("click",function(){
     ...
     //liste libre
     var re1=canvas.append("rect").attr("width",150).attr("height",25).attr("fill","#B0EFC1")
     .attr("x",40).attr("y",0).attr("ry",6).attr("data-step",8).attr("data-intro","la liste des partition libre de la mémoire ")
     var tx2=canvas.append("text").text("Les partitions libres").attr("x",45).attr("y",15).attr("fill","black").attr("font-size",12).attr("font-family","monospace")
