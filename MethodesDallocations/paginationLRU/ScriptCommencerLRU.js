window.onload=function(){
    ...
    d3.select("#exemple").on('click',exemple);
...
function commence () {
            ...
            fifo.cacherFifo(0);
            mm=new MmuLRU(nbC,nbP,memC,memV,fifo);
    ...
    d3.select('#suivant').on('click',function(){
        ...
        {   
            ...
            d3.select('#rect3').transition()
                               .delay(2000)
                               .style("opacity","0")
            let nbdep=fifo.calculDep(tab[i]);
            let dure;
                               if(nbdep===1)
                               {
                                  dure =500;
...
function exemple(){
    ...
    d3.select("#svg").attr("viewBox","220 200 900 650");
    d3.select("#affichage").style("visibility","visible");
    memC=new MemoireCent(svg,'grey',971,440,5,150,30,971+164,465,"18px",400,'black',3,3,'#C4C4C4C4','#C4C4C4C4');
    memV= new MemoireVirt(svg,'grey',234,440,5,150,30,290,465,"18px",400,'black',5,3,'#C4C4C4C4',"rgb(28,200,138)");
    demande= new Rectangle(svg,'rgb(236,104,94)',299,425,5,20,15,0,0,0,0,'black');
    p= new Rectangle(svg,'rgb(54,185,204)',1037,330,5,20,15,0,0,0,0,'black');
    p.visibility('hidden',0);
    d3.select("#chaineText").style('visibility','visible');    
    ...
    intro.onexit(function(){
        Swal.fire({
            title: 'Exemple',
            html: "Dans cette exemple animé, les données utilisées sont : <br> - Nombre de page : 5 <br> - Nombre de cases: 3 <br> - Chaine de référence : 0, 1, 3, 0, 2.",
...
function commenceExemple()
{
    ...
    simulation=true;
    termineAble=true;
    d3.select("#chaine").attr("data-step","8").attr("data-intro","La chaine de référence entrée");
    d3.select("#suivant").style("visibility","visible").style("cursor","pointer").attr("data-step","9").attr('data-intro',"Appuyer pour commencer l'exemple ");  
    commencer = false;
    let introSuiv=introJs();
    introSuiv.onexit(function(){
        d3.select("#suivant").attr('data-intro',"Appuyez pour référencer la page suivante dans la chine de référence");
        
    })
    introSuiv.goToStepNumber(8).start();
    tabRef=new TableRef(tab,svg,1000,650,nbC); 
    {   //Dessiner les fenetres
         flou =  svg.append('rect')
            .attr('height',3000)
...
if(i>=18){
    ...
    tour2=introJs();         
    d3.select("#terminer").attr("data-step","9").attr('data-intro',"Appuyez pour terminer la simulation et afficher le bilan");
...
let id= setInterval(function(){
    ...
    {   
        ...
        setTimeout(function(){able=true; i++; t++;j++;
        // introJs suivant
        switch(i)
        {
            case 1: { tour2=introJs();         
             ...
             break;
              case 2:{
               tour2=introJs();         
               d3.select("#file").attr("data-step","9").attr('data-intro',"Vous pouvez afficher la file des pages résidantes en mémoire centrale à la volonté");
               d3.select("#suivant").attr("data-step","10").attr('data-intro',"Appuyez sur le boutton pour avancer dans la chaine de références");
               tour2.goToStepNumber(9).start();
               tour2.onexit(function(){
                   d3.select("#file").attr("data-step",null).attr('data-intro',null)
                   d3.select("#suivant").attr("data-step",null).attr('data-intro',null);
                   tour2=null;
                   able=true;
               })
                  ...
                  case 4:
                      ...
                      case 5:
                       {
                           tour2=introJs();         
                           d3.select("#ajouterPage").attr("data-step","9").attr('data-intro',"Tant que le nombre de pages référéncées ne dépasse pas 18, vous pouvez référencer d'autre page d'ici");
                           tour2.goToStepNumber(9).start();
                           tour2.onexit(function(){
                               d3.select("#ajouterPage").attr("data-step",null).attr('data-intro',null);
                               tour2=null;
                               able=true;
...
{   
    ...
    fifo.textFifo('File des pages en MC',3000);
    fifo.apparaitreFifo(3500);
    fifo.etatFifo("Chargement de la page "+tab[i],3500);
    mm.chargerpage(tab[i],idC,t,1000,4000);
    let id= setInterval(function(){
        if(pret)
        {   
                    ...
                    tour2.onexit(function(){
                       ...
                       case 4:
                           ...
                           case 5:
                            {
                                tour2=introJs();         
                                d3.select("#ajRef").attr("data-step","9").attr('data-intro',"Tant que le nombre de pages référéncées ne dépasse pas 18, vous pouvez référencer d'autre page d'ici");
                                tour2.goToStepNumber(9).start();
                                tour2.onexit(function(){
                                    d3.select("#ajRef").attr("data-step",null).attr('data-intro',null);
                                    tour2=null;
                                    able=true;
...
function creerTabExemple(){
    if(able){
       ...
       btn.on("click",function(){
           window.location.href="paginationLRUsansExemple.html";
       })
   }
}
