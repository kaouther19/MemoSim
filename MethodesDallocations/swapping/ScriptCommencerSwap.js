...
if((t<=0) ||(t1<=0)||(document.getElementById('taips').value=="")||(document.getElementById('tmps').value=="")||document.getElementById('nbp').value==0)
{
    ...
    {
        Swal.fire("Veuillez introduire la taille et le temps")
    }
    else
    {
            if(document.getElementById('taips').value=="")
                        {
                            Swal.fire("Veuillez introduire la taille")
                        }
              else       
                        {
                        ...
                        if (g == (nbPart-1 )) {
                         ...
                         else
                          {
                            ...
                            if ( tabTaillePart[g] >= taille - ancien) {
                                ...
                                Swal.fire("La taille de la partition " + g + "doit étre inférieure à " + (taille - ancien));
...
d3.select('#commencer').on('click', function() {
    ...
    else
    {
        ...
        {
              Swal.fire('Introduisez les information du processus et des partitions')
        }
        if (v1==false)
        {
        ...
        if(v2==false)
        { 
            Swal.fire('Inreoduisez les informations du partitions')
        }
    }
...
var clock= setInterval(async function(){
 ...
 ////////////////////////////
    ...
    else
    {
        ...
        if(p.taille>max)
        {
       Swal.fire("La taille de ce processus est supérieure à la taille de toutes les partitions de la mémoire ")
        var p = d.donnerPsus()
...
else
{
  ...
  d3.select('#ajoutok').on('click', function() {
    ...
    if((t<=0) ||(t1<=0))
    {
        if(t<=0)
        {
            Swal.fire("La taille doit étre supérieure à 0,Veuillez introduire les attributs")
        }
        if(t1<=0)
        {
            Swal.fire("Le temps doit étre supérieur à 0,Veuillez introduire les attributs")
        }
    }
    else
    {
...
function exemple()
{
   ...
   let fait=true;
   var ec=canvas.append("ellipse").attr("cx",465).attr("cy",441.5).attr("rx",65).attr("ry",16.5).attr("fill","#333333").attr("data-step",17).attr("data-intro","le disque dur")
   var ec1 = canvas.append("rect").attr("x",400).attr("y",410).attr("width",130).attr("height",29).attr("fill","#363131")
   var ec2 = canvas.append("ellipse").attr("cx",465).attr("cy",412.5).attr("rx",65).attr("ry",22.5).attr("fill","#857575")
...
/////////////////entete de la table d'etat
 var re=canvas.append("rect").attr("width",190).attr("height",25).attr("fill","#D6CFD2").attr("x",170).attr("y",-17).attr("ry",6)
 var t1=canvas.append("text").text("Table d'état").attr("fill","black").attr("x",220).attr("y",-3).attr("font-size",14).attr("font-family","monospace")
 //entet table etat
 var lin=canvas.append("line").attr("x1",48).attr("y1",32).attr("x2",283).attr("y2",32).attr("stroke","#0C0101").attr("stroke-width",1.5)
  var entidtxt = canvas.append("text").attr("x",50).attr("y",27).text("id").attr("fill","black").attr("font-size",14).attr("data-step",10).attr("data-intro","l'identificateur de la partition")
  ...
  into.onexit(function()
              {
                ...
                console.log('------------------------------------');
                  Swal.fire({
                    title: 'Exemple',
                    html: "Dans cette exemple animé, les données utilisées sont : <br> - taille de la mémoire : 300 ko <br> - Nombre de partitions: 4 <br> - les partitions :partition1(100) ,partition2(80), partition3(60),partition4(60)<br> - Nombre de processus: 3 <br> - les processus(taille,temps) :p0(40,20) , p1(60,30) , p2(100,45) <br> pendant l'excutuion on bloquera le processus P1 <br> ",
                    ...
                    taille=300;
                    nbPart=4;
                    tabTaillePart=([100,80,60,60])
                    tabtai=([40,60,100
                    ])
                    tabtem=([20,30,45])
                    dataTab.push([1,40,20,1,100,true])
                    dataTab.push([2,60,30,2,80,false])
                    dataTab.push([3,100,45,1,100,false])
                    ...
                    var n=0;
                    var clock= setInterval(async function(){
                        if((m.tabMem[1].etat == "libre")&&(fait2==true))
                        {
                            Swal.fire('supposons qu on veut déblouquer P1')
                            db(1)
                            fait2=false

                        }
                        if((m.tabMem[1].etat == "occupé")&&(m.tabMem[2].etat == "occupé")&&(fait==true)) 
                        {
                            Swal.fire('supposons qu on veut blouquer P1 afin que P3 prends sa place')
                            ...
                            //  d.insertPsus(p);
                              m.liberer(1,canvas)  
                              console.log('------------------------------------');
                              console.log('hhhhhhhhhhhhhhhhhhhhhhhhhhhhhannnnnniiiiiiiiiiiiiiiiiiiii44444444444444444444444444');
                              console.log('------------------------------------');
                              fait=false;
                              fait2=true;
                        }
